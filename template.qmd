...
a <- rnorm(10000) #rnorm generates random numbers on a set range [1:10000]
b <- rnorm(10000)
...
#I think the replacement argument puts back already selected observations back into the pool allowing them to be possibly be selected again
...
Based on the AIC and RMSE scores of the cross validation model 2 does the best job of predicting penguin bodymass.

The final step is to run the selected model on all the data. Fit a final model and provide the summary table.

```{r}
final_model = lm(f1, penguins_clean)
